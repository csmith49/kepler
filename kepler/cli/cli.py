...
@cli.command()
@click.option("--status", "-s", type=click.Choice(["running", "completed", "error", "interrupted"]), 
              help="Filter by status")
@click.option("--tag", "-t", multiple=True, help="Filter by tag (can be used multiple times)")
@click.option("--sort", type=click.Choice(["name", "start_time", "end_time", "status"]), 
              default="start_time", help="Sort by field")
...
def list(status: Optional[str], tag: List[str], sort: str, reverse: bool):
    ...
    for exp in experiments:
        ...
        status_str = f"[{status_colors[exp.status]}]{exp.status}[/{status_colors[exp.status]}]"
...
def delete(experiment_id: str, force: bool):
    ...
    # Confirm deletion
    if not force and not click.confirm(f"Are you sure you want to delete experiment '{exp.name}' ({exp.id})?"):
        console.print("Deletion cancelled.")
        return
    
    # Delete the experiment
    if delete_experiment(experiment_id):
        console.print(f"[green]Experiment '{exp.name}' ({exp.id}) deleted successfully.[/green]")
    else:
        console.print(f"[red]Failed to delete experiment '{exp.name}' ({exp.id}).[/red]")
...
def dir():
    ...
    if not app_dir.exists():
        console.print("[yellow]Directory does not exist yet. It will be created when you run your first experiment.[/yellow]")
        return
